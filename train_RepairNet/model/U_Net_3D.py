...
class unet3dUp(nn.Module):
    def __init__(self, in_channels, out_channels, batch_norm=True, sample=False):
        ...
        self.sample = nn.ConvTranspose3d(in_channels, in_channels, 3, stride=2)
...
class unet3dUp1(nn.Module):
    def __init__(self, in_channels, out_channels, batch_norm=True, sample=False):
        super(unet3dUp1, self).__init__()
        self.pub = pub(in_channels // 2 + in_channels, out_channels, batch_norm)
        self.sample = nn.ConvTranspose3d(in_channels, in_channels, 4, stride=2 ,padding=1)
    def forward(self, x, x1):
        x = self.sample(x)
        x = torch.cat((x, x1), dim=1)
        x = self.pub(x)
...
class unet3d(nn.Module):
    def __init__(self):
        ...
        self.up1 = unet3dUp1(128, 64, batch_norm, sample)
